...
async function handle(req) {
  switch (req.op) {
    ...
    case 'e2ee_encrypt_v2': {
      const seq = bigIntOrNum(req.sequenceNumber);
      const ptMode = req.ptMode || 'array';
      const ptBuf = Buffer.from(String(req.plaintextB64), 'base64');
      const plaintext = ptMode === 'buffer' ? ptBuf
        : ptMode === 'array' ? Array.from(ptBuf)
      ...
      try {
        const ct = await send({ command: 'e2eechannel_encrypt_v2', ltsmKeyId: req.channelId,
          payload: { to: req.to, from: req.from, senderKeyId: Number(req.senderKeyId),
            receiverKeyId: Number(req.receiverKeyId),
            contentType: Number(req.contentType),
            sequenceNumber: seq,
            plaintext } });
        return bytesToB64(ct);
      } catch (e) {
        throw new Error((e && e.message ? e.message : e) + ' | args ' + argTypes(req)
          + ` seq=${typeof seq} ptMode=${ptMode}`);
      }
    }
    case 'e2ee_encrypt_v1': {
      const pt = b64ToBytes(req.plaintextB64);
      try {
        const ct = await send({
          command: 'e2eechannel_encrypt_v1',
          ltsmKeyId: req.channelId,
          payload: pt,
        });
        return bytesToB64(ct);
      } catch (e) {
        throw new Error((e && e.message ? e.message : e) + ' | encrypt_v1');
      }
    }
    ...
    case 'e2ee_wrap_group_shared_key':
      // channel = e2eekey_create_channel(myKeyHandle, memberPubKey);
      // payload is the *group key handle* (number), not bytes — chrome registerGroupKey.
      return bytesToB64(await send({
        command: 'e2eechannel_wrap_group_shared_key',
        ltsmKeyId: req.channelId,
        payload: Number(req.groupKeyHandle),
      }));
    default:
